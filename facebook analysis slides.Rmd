...
2. 註冊App: 每個App都必需連到<https://developers.facebook.com/apps>註冊並取得對應的**App ID** (App的身份證字號)及**App Secret**(App開發者證明)  
    ...
    2.2 Fill in app info
    
    ![](./fig/newappinfo.png)
    
    2.3 Select a product: Facebook login
    
    ![](./fig/fblogin.png)
    
    2.4 Select platform: Web
    
    ![](./fig/selectplatform.png)
    
    2.5 Setup site URL: http://localhost:1410/ 
    
    ![](./fig/siteurl.png)
    
3. 取得App ID及App Secret
...
#fbOAuth()
R_app<-list(
  app_id="376441419449841",
  app_secret="41af732717539c4191b3a4fd135e8a86"
)
```
...
連回developer, 如下回位置填入`http://localhost:1410/ `, save changes.

![](./fig/sidebar_facebooklogin.png)

![](./fig/validoauthuri.png)
...
一旦儲存權杖後，以後library套件後，只需直接執行以下行就可，無需再經歷以上步驟。
